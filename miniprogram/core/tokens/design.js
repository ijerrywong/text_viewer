...
var SIZE = {
  // 页面左右留白（内容区到屏幕边缘）。首页所有区块 —— 导航标题、hero、
  // 入口卡、最近文件、提示卡、页尾脚注 —— 共用这一个值，页面才只有
  // 一条左边线；分散写成 space-8 时，任何一处改动都会让那条线断掉。
  // 48rpx，即间距阶梯的 space-12。
  // ⚠️ 它同时决定内容区可用宽：750 - 48×2 = 654rpx。hero 标题是单行不折的，
  //    654 / text-6xl(48rpx) = 13 字封顶，现在正好用满 13 字。
  pageGutter: 48,
  navBarHeight: 88,      // 自定义导航栏内容区高度
  navIcon: 64,           // 导航栏圆形按钮
  entryIcon: 80,         // 入口卡片图标框
  // 首页三张入口卡的统一高度。重复原则要求这一组卡片是同一个矩形，
  // 所以高度锁死，不让任何一张随自己的文字长短浮动：
  //   上下内边距 space-8 ×2                     = 64
  //   标题 text-2xl(32) × leading-tight(1.4)    = 44.8
  //   标题下 margin space-1                      =  4
  //   描述 text-sm(24) × leading-normal(1.6)    = 38.4
  //                                        合计 ≈ 151.2
  // 取 156 留 4.8rpx 余量，只够抵消 rpx→px 的舍入 —— 算少了内容会顶出去，
  // 等高当场破掉；算多了三张一起长高，白吃首屏。
  // ⚠️ 改动 .entry-title / .entry-desc 的字号或行高要回来重算。
  // ⚠️ 这个高度只够两行。想在卡里加第三行（比如曾经的剪贴板标注）就得连
  //    这个值一起改，否则新文字会把卡顶变形。
  entryCardHeight: 156,
